...
採用済みの拡張（#122）として、`gui.background_image_dir`を指定すると、ローカルのPNG/JPEG画像をウィンドウごとに1枚選び、薄く背景表示する。利用可能な画像がない場合は黒背景に戻す。操作部品や画像投稿機能は追加しない。配置・不透明度・読み込み上限は[背景の仕様](gui-background.md)で定める。
