...
- [x] T003 [P] Add and pin dependencies `fyne.io/fyne/v2@v2.8.1`, `github.com/pelletier/go-toml/v2@v2.4.3`, `github.com/oklog/ulid/v2@v2.1.2` in `go.mod` and commit `go.sum`
  ...
  - `github.com/pelletier/go-toml/v2@v2.4.3` is pinned as a **direct** requirement as of the settings batch (T016), with `go.sum` populated. `github.com/oklog/ulid/v2@v2.1.2` is pinned as of the orchestrator batch (T025). `fyne.io/fyne/v2@v2.8.1` is pinned directly by Batch 7; all three pins are present and `go.sum` was committed in Batch 7 commit `9c3e47f`.
...
- [x] T032 [US1] Implement the append writer using `O_APPEND|O_RDWR|O_NOFOLLOW` plus `O_CREATE` only when `create_if_missing` is true, never `O_TRUNC`, writing UTF-8 with exactly one trailing LF, in `internal/sink/obsidian/sink.go`; refuse a note path that is a symbolic link or not a regular file, write one leading LF when the note's last byte is not one so the entry starts its own physical line, report a missing daily-note directory as `ErrVaultMissing` rather than the `create_if_missing` refusal, and do **not** search for, create, or insert into a named section (FR-045, FR-046, FR-048 – FR-050; contracts/obsidian-sink.md as amended in Batch 6a)
  - **`post.Targeter` landed with this task**, since T030 is where the path is resolved and this batch is the first to have a type that implements it (issue #98). The obsidian sink recorded what `Send` actually wrote and returned that, never re-resolving. Against #98's four acceptance boxes: box 4 (`Sink` stays two methods, the target interface separate and optional) is **discharged**; box 2's midnight property is discharged **at the sink** but its logged half needed T040; boxes 1 (`path` on the three obsidian events) and 3 (chat events carry none) were **both** T040, and box 3 additionally needed the telegram sink to exist before anything could assert it.
  - **Superseded by T040 (DEC-D3).** `Targeter`'s getter held the *sink's* most recent note rather than the calling post's, which is issue #111 — and it could not put a path on `obsidian_append_started` at all, since that event precedes `Send`. T040 replaced it with `post.ReportTarget` on the context and `post.TargetReporting` as a declaration with no getter, and removed `Target`, its mutex and its field from this sink.
...
- [x] T034 [P] [US1] Implement Telegram response decoding of `ok`, `error_code`, and `description`, in `internal/sink/telegram/response.go` (FR-035, FR-066)
  - Decoding fails closed at every step, and the decoded fields are carried on an exported `*telegram.APIError` reachable through `errors.As`. That type is also how `http_status` gets out of a sink whose `Send` returns a bare error — the same shape gap the target reporter closes for `path` — so T040 has a route to the field contracts/log-events.md requires. T040 reads it with `errors.As`, which walks past this package's redacting wrapper.
...
- [x] T035 [US1] Implement `Send` performing the single MarkdownV2 attempt bounded by the configured request timeout, in `internal/sink/telegram/sink.go`; do **not** add a queue or automatic re-send (FR-019, FR-033, FR-040, FR-041)
  ...
  - The sink deliberately does **not** implement the optional target interface — `post.TargetReporting` after DEC-D3, `post.Targeter` before it: chat events carry no `path` (issue #98, box 3). A test asserts the negative, since Go cannot express it as a compile-time assertion.
...
- [x] T037 [US1] Implement flag parsing, message-argument joining with exactly one ASCII space, and dispatch between posting and the window, in `internal/cli/cli.go` (FR-002 – FR-004, FR-008)
  ...
  - **`--config` is dropped at the parse boundary for the window path** (FR-005), so the window is handed nothing to misuse rather than being trusted to ignore it. FR-006's error is T079 and is **not** implemented; `mp -c x.toml` therefore opens the window path on the default settings today. Recorded in `contracts/cli-interface.md` under **Not yet implemented**. *(Superseded by Batch 11: T079 is delivered — `mp -c x.toml` now errors without opening the window, and the "Not yet implemented" contract section is replaced by "Verification boundary".)*
  - `-h`/`--help` is recognised and refused with a message saying help is unimplemented, rather than reported as an unknown flag, which would read to a user as their own typo. T080 owns the real output; the contract's exit `0` is not met today. *(Superseded by Batch 11: T080 is delivered — help prints and exits `0`.)*
...
- [x] T039 [US1] Implement the wiring of settings, logging, and the posting service with exactly **one** `os.Exit` call site computing the status from the aggregate — this single binary providing both front doors is what satisfies FR-001 — in `cmd/mp/main.go` (dispatch and the exit), `internal/app/app.go` (construction) and `internal/cli/run.go` (the CLI run sequence) (FR-001, FR-059, FR-060)
  ...
  - **FR-018's startup error is not implemented** (T081). Both destinations disabled reports `No destination is enabled, so nothing was posted.` after the fact and exits `1`, rather than refusing beforehand with an actionable message. *(Superseded by Batch 11: T081 is delivered — both destinations disabled is refused before a post is attempted.)*
- [x] T040 [US1] Emit `message_received`, the four sink start/succeed/fail events, and `request_completed`/`request_completed_with_error` from the orchestrator, in `internal/post/service.go` (FR-067)
  - **Batch 6c-2.** Under DEC-D2 the emission goes through a domain-shaped `post.Recorder` declared in `internal/post`, with the mapping onto `logging.Event` in `internal/app/recorder.go`, so `internal/post` keeps importing no other internal package — `internal/logging` imports `internal/config`, so `post -> logging` would reintroduce `post -> config` transitively. `internal/app/layering_test.go` fails if it does.
  - **`post.Targeter` is gone, replaced by a per-call reporter on the context (DEC-D3).** `post.ReportTarget(ctx, path)` is what the obsidian sink calls the instant it resolves the note, before any I/O, and the orchestrator emits `obsidian_append_started` at that call with the path attached. `post.TargetReporting` survives as the declaration the orchestrator type-asserts — whether to hold a sink's start event for a report that is coming — and carries no getter, because a getter on a sink that serves every post can only report the sink's most recent note. That closes **#98 boxes 1 and 3** and **#111** by construction: the value never leaves the goroutine running the post. `Sink` is still exactly two methods (#98 box 4).
  - **`error_type` is emitted now, with two values.** `post.ErrorType` returns `timeout` or `failed` from the same `errors.Is(err, context.DeadlineExceeded)` predicate `reasonFor` uses, so the log's classification and the display reason cannot drift; a test pins that they partition failures identically. T056 widens the set rather than changing what these two mean, so a query written today for `timeout` keeps working. The alternative — omitting a field FR-066 requires until T056 — was rejected because an absent field is a query that silently returns nothing.
  - **`message` is knowingly not emitted, and that is FR-068 unmet rather than an oversight.** The capture rule (record the body whenever any enabled sink failed) needs `message_on_error_only` and the post's outcome, and is T071. `message_len` and `message_bytes` are emitted on `message_received` as R-010 defines them — a rune count and a byte length, asserted numerically against CJK text so one cannot be substituted for the other. T072 stays open for whichever other records need them.
  - **`http_status` comes off `*telegram.APIError` through `errors.As`**, which walks past the sink's own redacting wrapper. The nil check after `errors.As` is not redundant: a typed nil in the chain matches and leaves the target nil.
  - **#110 is closed by keeping the value rather than by adding vocabulary.** `nameOf` returns the recovered panic alongside the name, wrapped in the same `panicError` a panicking `Send` produces. It cannot ride on a lifecycle record — a panicking `Name` resolves to the sentinel, and the event names are keyed by sink name — so the adapter reports it on the post's terminal record, which is the one record always written. `Success` still follows what `Send` did (FR-019).
  - **An unmappable sink name is reported rather than dropped.** The sentinel, and any third sink added without extending the adapter's table, produce no lifecycle records at all; both cases are named in the terminal record's `error` field instead of vanishing. The terminal record's own name and level still track `AllSucceeded`, so the exit status and the log agree.
  - **DEC-D2's mandatory obligation is discharged**: `internal/logging`'s registration test scans its own source and cannot see the adapter, so `TestTheAdapterProducesEveryOrchestratorReachableEvent` compares the adapter's producible set against `logging.AllEvents()`, with the three formatting-fallback names listed as deferred to T063. A name added to `logging` and not mapped here fails.
  - **A defect in `internal/logging` had to be fixed for these records to satisfy the contract.** `Options.Redact` populated — which `app.OpenLogger` always does, so every real run since 6c-1 — made the redacting `ReplaceAttr` consume the level attribute before the rename could see it: `slog.Level` implements `fmt.Stringer`, so the scrub stringified it to `INFO` and the lowercasing silently stopped. Every shipped record carried `INFO`/`ERROR` while `contracts/log-events.md` admits only `info`/`error`, and the package's own lowercase test passed because it configures no credential. The rename now runs first, pinned by a test that arms `Redact` and asserts both the level and that the scrub still fires.
  - **A recording can neither block nor break a post.** Every recorder call is panic-guarded, `New` accepts a nil `Recording`, and a `Recording.Post` that panics or returns nil (or a typed nil) is contained — FR-076's first duty is that a diagnostics failure changes nothing about the post.
...
- [x] T066 [P] [US5] Write message-capture tests asserting a fully successful post records no body while a failed post records the original body, in `internal/logging/logger_test.go` (FR-068, SC-008)
  - **File path corrected.** Delivered in `internal/app/capture_test.go`, not `internal/logging/logger_test.go`. Under DEC-D2 the capture rule lives in the record-mapping adapter, so its tests drive a whole post through a real `logging.Logger` and read the bytes back — `internal/logging` never sees a message body or a post outcome.
  - **The rescue path was the defect this task found.** A first implementation attached the body to every failure-shaped record, including `telegram_markdown_failed`, which FR-039's plaintext rescue emits inside a post that then *succeeds* — so every rescued post logged the user's private text. `TestARescuedPostIsASuccessAndRecordsNoBody` is that case, and the fix is DEC-G1's narrowing to `SinkFinished`'s records.
  - **The assertions are on presence and absence, not values alone.** The record shape changed in this batch and the entire pre-existing suite stayed green, because nothing asserted field *completeness* on a failure record.
- [x] T067 [P] [US5] Write a degraded-logging test asserting that with an unwritable log path every sink still runs, real outcomes are reported, the exit status is unchanged, and **exactly one** warning is emitted, in `internal/post/service_test.go` (FR-076, SC-013)
  - **File path corrected.** Delivered in `internal/app/degraded_path_test.go`, not `internal/post/service_test.go`: the posting core imports no `internal/logging` and cannot be handed a logger, so "the log could not be opened" is not a condition it can be placed in.
  - **Three of FR-076's four clauses are asserted there together** — every sink still runs, the real outcomes are reported, the exit status tracks the post and not the log. The fourth, *exactly one* warning, is not observable in `internal/app`, which has no user-visible output; it is carried at the two front doors, by `internal/gui/degraded_test.go` across three posts in one session and by the pre-existing `internal/cli/cli_test.go` tests.
...
- [x] T071 [US5] Implement the `message_on_error_only` capture rule — omit the body on full success, include it when any sink failed — in `internal/logging/logger.go` (FR-068)
  - **File path corrected.** Implemented in `internal/app/recorder.go`, not `internal/logging/logger.go`. The rule needs the post's outcome and the setting, neither of which `internal/logging` has: it receives finished records. T040's own note above already assigned the rule here.
  - **DEC-G1**: the body goes on each `*_failed` record from `SinkFinished`, and on the terminal record only when no failure record could carry it — the shape a panicking `Name` produces, where no lifecycle record exists at all. The formatting-fallback records deliberately carry no body; see T066.
- [x] T072 [US5] Record `message_len` as a rune count and `message_bytes` as the UTF-8 byte length on successful posts, in `internal/logging/logger.go` (resolves A-010, FR-068)
  - **File path corrected, and no code was changed.** The fields are emitted from `internal/app/recorder.go`'s `MessageReceived`, delivered by T040 with R-010's semantics already correct.
  - **Closed by verification rather than by inspection.** Three mutants were built — a byte count under `message_len`, a rune count under `message_bytes`, and the two swapped — and all three are killed by the existing `TestASuccessfulPostWritesTheContractsRecords`. The task is complete because the behaviour is right *and* pinned.
  - **T040's residual is retired, not dropped.** That note left this task open "for whichever other records need them". The answer is none: the counts describe the post, `message_received` is emitted exactly once per post, and every record of that post shares its `message_id` (R-007), so a consumer joins rather than re-reads. Repeating them on each sink record would be the same number restated per destination.
- [x] T073 [US5] Record stack traces for panics and unexpected errors subject to the `stack_trace` setting, and **never** manufacture a trace for an expected operational error, in `internal/logging/logger.go` (FR-071)
  - **File paths corrected.** Split across `internal/post/service.go` (the capture) and `internal/app/recorder.go` (`traceFor`, `keepTrace`, `terminalAttrs`, the `stack` key). Nothing landed in `internal/logging/logger.go`.
  - **DEC-G3**: the stack is taken by `debug.Stack()` inside the deferred `recover`, which is the last moment the frames exist — `recover()` returns the panic value alone. `SinkAttempt.NameErr`'s comment is the precedent for a T073-motivated change at the recovery point. It is read back through the new `post.Traced` interface rather than an exported error type, because the recovered value beside the trace can hold a credential.
  - **DEC-G8: "unexpected errors" yields no `stack`, and that is the requirement met rather than deferred.** FR-071 qualifies all three of its categories with "where a trace is **available** and useful". A plain Go error carries no frames, and the same requirement forbids inventing any, so the absence of the field *is* compliance. A panic is the only failure this program can obtain real frames for. A future error type that captures its own stack needs only to implement `post.Traced`.
- [x] T074 [US5] Surface a single warning naming the log path and reason through the front door in use when diagnostics cannot be written, without altering reported outcomes or the exit status, in `internal/cli/render.go` and `internal/gui/result.go` (FR-076, A-012)
  - **`internal/cli/render.go` was not touched, deliberately.** The CLI half already worked, through `closeAndWarn`/`warningFor`, with its own tests. Only the GUI half was missing: `internal/gui` never called `Logger.Degraded()`, so diagnostics could fail for a whole session with the user never told (issue #75). Adding a second CLI code path to satisfy a file list would have been a change with no defect behind it.
  - **DEC-G5**: the GUI warns once per *session*, not per post. The logger latches its degradation and the window outlives every post, so re-rendering it would report one failure many times — not what FR-076's "exactly one" or SC-013's "exactly once" mean. A degradation that begins mid-session is still reported, on the first post that could have been affected.
  - **DEC-G6**: the warning replaces the `Details: <log path>` line rather than joining it, because pointing a user at a log for details when the log is what failed is an instruction to read a file that does not have them.
...
- [x] T082 [US6] Implement the minimal startup-error window showing the actionable message and the resolved settings path, offering no message field, exiting `1` when dismissed, in `internal/gui/errorwindow.go` (FR-030) — depends on the `internal/gui` package from US2
  - **Delivered in Batch 11, verified headlessly only.** `internal/gui/errorwindow.go` and `startupFailure` in `run.go`: message, resolved path, no field, and Quit / close box / Esc / Cmd+Q, with Esc routed through the focused `commandButton`. Exit `1` after dismissal is asserted with Fyne's headless app. **Native dismissal was not observed** — osascript was blocked on Accessibility permission — and cycle 0 found Esc dead in the real driver while a headless test passed, so a native check is still owed, by T091 / #92. With no resolvable path, no window is opened (stderr, exit `1`), so Fyne never writes storage relative to the working directory.
