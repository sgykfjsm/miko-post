...
- [x] T033 [P] [US1] Implement the Telegram request builder with an injectable base URL defaulting to `https://api.telegram.org`, in `internal/sink/telegram/request.go` (FR-031, FR-032, research R-008)
  - **The body is `application/x-www-form-urlencoded`, not JSON, and that is a decision (issue #113).** `json.Marshal` substitutes U+FFFD for bytes that are not valid UTF-8 and returns a nil error, so a JSON body would have made Telegram store different text from the obsidian note for one post — #113's option 3, rejected in advance. Form encoding is byte-preserving.
...
- [x] T035 [US1] Implement `Send` performing the single MarkdownV2 attempt bounded by the configured request timeout, in `internal/sink/telegram/sink.go`; do **not** add a queue or automatic re-send (FR-019, FR-033, FR-040, FR-041)
  - **No error `Send` returns carries the bot token.** The credential is in the request path, so net/http's `*url.Error` renders it verbatim and it would land in `post.SinkResult.Err`, which `internal/post/result.go` deliberately leaves unredacted. Every `*url.Error` layer is stripped before wrapping, with a textual net behind it, and a sentinel-token sweep asserts absence across `Error()`, `%v`, `%+v` and `%#v` for every failure class.
  - **`http_timeout_seconds` is clamped at the conversion point (issue #109).** T035 is the first code in the repository to turn a settings seconds value into a `time.Duration`; `18446744074` passes validation and wraps to a positive ~290ms deadline. Handled inside this package to keep the batch's footprint to itself — #109 should be amended to cover `http_timeout_seconds` as well as `sink_timeout_seconds` and to reject the value where the user can be told.
