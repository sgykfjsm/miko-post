...
|---|---|---|---|
| `enabled` | bool | `false` | — |
| `bot_token` | string | — | Required when enabled and `MIKO_POST_TELEGRAM_BOT_TOKEN` is unset. Never logged or printed (FR-043). |
...
| `http_timeout_seconds` | int | `30` | > 0 (FR-040) |
...
| Key | Type | Default | Validation |
|---|---|---|---|
| `sink_timeout_seconds` | int | `60` | > 0. Applied **independently** to each sink (FR-015). |
