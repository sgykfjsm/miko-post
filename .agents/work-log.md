...
- Test-quality findings applied: `fieldOf` now fails loudly on duplicate event names rather than silently checking the first (COR-003); a dominated, unfailable assertion dropped from T067's test (COR-004); `TestDefaultsKeep…` renamed to say what it actually asserts, since the old name promised a record-level property its body never checked (COR-005).
