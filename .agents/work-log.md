...
- Published as **PR #131** (https://github.com/sgykfjsm/miko-post/pull/131) from `sgykfjsm/batch-10a-records` at `e0a0c39` against `main` `0e3df5e`: one commit, four files, all under `.agents/`, `mergeable_state: clean`. `closingIssuesReferences` verified **empty** after opening — the body names nine issue numbers and must link none of them, since the four it reports on are already closed. Checking rather than assuming is the standing rule here; this is the third time that check has been the thing that confirmed a PR's linkage was what was intended.

### 2026-09-18 — Merge PR #131, rebase Batch 10b onto it, push
- User authorised merging #131 and rebasing and pushing 10b. **PR #131 squash-merged as `86a05f57c9e258355d3d2c54dc75d91dbbb5f072`**, so the 10a record reconciliation is now on `main` on its own, decided as its own reviewable unit. Verified afterwards that `main`'s `.agents/` content is byte-identical to `e0a0c39`'s.
- **Rebased 10b with `git rebase --onto origin/main e0a0c39`**, which drops the records commit explicitly rather than relying on git noticing the patch was already applied. Seven commits replayed with no conflicts.
- **The rebase is verified not to have moved content**: the tree is `b09891ce208fcde2e5a6549a017144cd0c9c54ac` both before and after, and `git diff backup/10b-pre-rebase HEAD` is empty. A rebase is supposed to move history and leave content alone, and "supposed to" is not evidence. `backup/10b-pre-rebase` is kept at the old head.
- The long-standing excluded `.specify/integrations/claude.manifest.json` edit blocked the rebase as an unstaged change. It was copied to the session scratchpad, the file restored from HEAD, and the edit put back afterwards — deliberately **not** `git stash`, because the stash stack is shared with other worktrees and other sessions can pop it.
- `make check` and the native darwin/arm64 build were **re-run on the rebased tree** rather than trusted from before the rebase. Green.
- Branch pushed. No PR opened: the authorisation was to rebase and push. `.agents/batch-10b-pr-body.md` is prepared and current.
- **DEC-ADV-A filed as issue #132** (https://github.com/sgykfjsm/miko-post/issues/132), following the house format used for #127–#130: type and provenance header, the measured evidence, why it is not a blocker for the batch that found it, three options with costs, a recommendation, and acceptance criteria. Recommendation is option 2 — truncate at a documented cap with a `message_truncated` marker — because option 3 solves the amplification but not the single-record overshoot, which is the half that makes `rotate_size_mib` not mean what a user would assume. The acceptance criteria include one the review did not raise: if a cap is introduced it must not operate *after* redaction, or it could split a credential across the truncation boundary.
- **Batch 10b opened as PR #133** (https://github.com/sgykfjsm/miko-post/pull/133) from `06d2c1d` against `main` `86a05f5`: nine commits, 27 files, +2780/-68, mergeable. The body is `.agents/batch-10b-pr-body.md`, which already carried all three stages' outcomes and CON-002's resolution, so publication needed no rewrite.
- `closingIssuesReferences` verified **empty**, which is the intended state and is said so in the body: #67, #68 and #72–#75 stay open until review and merge, making their closure an explicit post-merge action. The check is not about wanting the field empty — it is about confirming the linkage matches the intent, because the same field being empty *unintentionally* is what left #66 and #69–#71 open for two days after #126.
- **Post-merge actions now owed on #133**, and they are the ones this project keeps dropping: close #67, #68 and #72–#75 with a merge reference, and sweep the older merged-but-open issues (#60–#65, #115, #118, #119).

### 2026-09-24 — Merge Batch 10b, close its six issues, reconcile the records
- User authorised merging PR #133 and closing its issues. Six days had passed since the branch was pushed; verified before acting that `main` was still `86a05f5`, that #133 was `OPEN`/`MERGEABLE`/`CLEAN`, and that its head was still `a944dc9` — the exact tree the three review stages examined.
- **Squash-merged as `d6fcb6bd8ce1b138d5659dfa095384866cf24684`.** The merged tree and the reviewed head share tree `9c81f88bf10d83c39aed87cfe318a4356eb046bb`, checked at merge time rather than asserted. `main` carries byte-for-byte what the review produced.
- **Closed #67, #68, #72, #73, #74 and #75 in the same session as the merge**, each with a comment naming PR #133 and `d6fcb6b`, the delivering files, the decisions that governed them, and the caveats carried forward: #72 points at #132 for the unbounded body, #73 records that its closure rests on three mutants rather than on inspection, and #75 records that the batch's most serious defect was in the fix for that very issue. PR #133 carried no closing keyword by design and said so, so this was an explicit step with a named owner.
- **Doing the closure in the same session as the merge is the only arrangement that has actually worked here.** The same step deferred left #66 and #69–#71 open for two days after #126, and #60–#65, #115, #118 and #119 open since earlier batches. Open issues: 56 → 50.
- Records reconciled immediately rather than later, which is the lesson the whole #131 episode taught: the moment 10b merged, `main`'s `.agents/` said it was *in progress*. 10b moved from `in_progress` to `completed` with the merge facts, `merge_verification` and an `issues_closed` block; `in_progress` is now `[]`; `next_best_action` rewritten as Batch 11; `remaining_batches` trimmed; `last_updated` 2026-09-24. `project-status.md` overwritten with the outgoing copy archived as `project-status-before-batch-11.md`.
- `next_best_action` names two things to settle early in Batch 11 rather than leaving them to be rediscovered: **T081 is the requirement T039's own acceptance note records as not implemented** — both destinations disabled currently reports after the fact and exits 1 rather than refusing beforehand — and `contracts/cli-interface.md` is the contract T080 must match exactly. Verified that issues #76–#84 map to T075–T083 one-to-one before writing them down.
- GitHub MCP was unavailable this session (connection closed), so every GitHub operation used the `gh` CLI. Noted because the global tool policy prefers the MCP server, and the fallback was a connection failure rather than a missing capability.
- Next: Batch 11 (US6 settings resolution). Still owed and not done here: the older closure sweep (#60–#65, #115, #118, #119), and answers to #132 and 10a's #127–#130.
- **Swept the older merged-but-open issues.** Closed **#60–#65** (T059–T064, Batch 9 / PR #125, `1707382`), each with a comment naming the PR and merge commit. Verified before closing rather than assumed: each task is `[x]` in `tasks.md`, #125 reads back as merged, and the implementation is present in `internal/sink/telegram`.
- **Kept #115, #118 and #119 open, which is three of the nine I was asked to sweep.** They are review follow-ups describing real unfinished work, not bookkeeping, and closing them would have deleted it. Checked against `main` `d6fcb6b`: #115's invariant is still unenforced (`internal/post/sink.go`'s `Sink` comment says nothing about credential-free errors; no `SanitizeError` or shared assertion helper), #118's `Error()` scan in `(*Sink).safe` has neither a pinning fixture nor a comment recording it as intentionally unpinned, and #119's two-succeeding-sinks path is still unreachable from argv, with `telegramThatFailsBeforeTheNetwork` still the only Telegram-enabled process fixture.
- Each of the three now carries a comment recording the check and its date, so a future sweep does not re-derive the same verification or bulk-close them on the assumption that age means staleness. **#119 is worth deciding before Batch 11** — US6 is the CLI front door's remaining behaviour, so it is the batch that will be working in that seam anyway.
- Open issues: 50 → **44**.

### 2026-09-24 — Clear the decision backlog
- User asked to process the pending decisions. Seven were outstanding; all seven now have an answer, and the four whose answer was purely "accept and write it down" are closed the same day rather than being left as a second backlog.
- **Closed, decided and documented**: **#132** (the body is captured in full, no bound), **#127** (concurrent front doors scatter records; symlinked `logging.path` unsupported under rotation), **#128** (`rotate_after_days` only on darwin with a birth time), **#129** (the repair newline can cross a rotation). All four consequences are now in `contracts/config-schema.md`, beside the clamp paragraph they relate to, so a reader meets them in one place.
- **#132 went against my recommendation**, which was a documented cap with a `message_truncated` marker; the maintainer took SC-008's "re-sendable without consulting any other source" literally. Recorded as decided, not re-litigated. What it changed is that the documentation had to work harder: the load-bearing half of accepting no bound is that a user configuring `rotate_size_mib` learns it does **not** cap a record, and that a 15 MiB paste rejected by both destinations writes ~30 MiB of archives that FR-074 forbids reclaiming.
- **Open with scoped work**: **#94** (stamp version and commit through linker flags — documented in `docs/design.md` §14 and the Japanese counterpart; closes when T090 asserts against that path, Batch 12), **#119** (an internal seam on `cli.Run`, no user-visible setting — scoped into Batch 11), **#130** (the load-time upper bound — scoped into Batch 11, which works on settings validation anyway).
- **#94 needed no code.** `make install` has carried `-ldflags "-X …version=… -X …commit=…"` since the Makefile was written; the mechanism existed and only the instruction was missing, so a maintainer reaching for `go install …@latest` out of habit would have shipped `git_commit = "unknown"` to every user with nothing saying not to. Checked the Makefile before writing the doc rather than describing a process that did not exist.
- Mirrored the release note into `docs/design.ja.md` for consistency, without touching its `<module-path>` placeholder — that is #95's, with its own acceptance.
- Three of the seven are now Batch 11's concern (#119, #130) or Batch 12's (#94), which means the next batch has a slightly larger footprint than its task list implies. Recorded in `next_best_action` so it is not a surprise mid-batch.
- `make check` green. Open issues: 44 → **40**.
- **Opened the records-only PR as #134** (https://github.com/sgykfjsm/miko-post/pull/134): three commits, seven files, +435/−128, no `.go` file in the diff. Covers the 10b reconciliation, the issue sweep and the decision backlog, each reviewable as itself rather than riding on a code change. The body names one thing to actually scrutinise — the #132 documentation in `contracts/config-schema.md`, since that decision went against the recommendation and the whole weight of accepting no bound falls on whether a user configuring `rotate_size_mib` can see what it does not promise.
- **The closing-keyword check fired, and it caught a real one.** `closingIssuesReferences` came back as `[60]` on first open: the body's phrase "**Closed #60–#65**" begins with a closing keyword directly followed by a reference, so GitHub linked #60. Harmless in effect — #60 is already closed — but it is the exact failure this check exists for, arriving from prose describing a *past* closure rather than from an intended link. Reworded to "Issues **#60–#65** were closed", re-verified empty, and a regex sweep for any remaining `close[sd]/fix/resolve + #n` pattern in the body came back clean.
- Worth keeping: the danger is not only forgetting a keyword, it is writing English that *contains* one. Past-tense prose about closed issues reads naturally and links silently.

### 2026-09-24 — Record PR #134's merge
